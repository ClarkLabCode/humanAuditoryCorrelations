%% Start of script to run auditory psychophysical experiments (sweep "delta note" parameter)
...
while exist(['DataNote/',subInfo,'.mat'],'file') % make sure no overwriting is happening
...
mainN = 90; % number of trials for each "main" trial type in experiment

% Vector of delta note values
deltaNote_vector = [-6 -4 -2 -1 0 1 2 4 6];

% Initilaize empty vector
tmpSeq = [];

% Create random sequences of trial types (i.e., correlation and 
% displacement) and delta notes (see vector of 9 values above)
tmpSeq = [ones(mainN,1); 2*ones(mainN,1)];
tmp = rectpulse(deltaNote_vector,mainN/length(deltaNote_vector))';
tmpDeltaNoteSeq = repmat(tmp',1,2)';
...
params.pipDeltaT = 0.04; % duration of pips in ms
...
for n = 1:nTrials
    ...
    % Vary delta note
    params.deltaNote = DeltaNoteSeq(n);
    ...
    toneStim(n) = pipDeltaNoteFunctionExp2Note(params);
...
try
    ...
    data = struct('rt',nan(nTrials,1),'choice',nan(nTrials,1),'rt_violation',nan(nTrials,1),...
        ...
        'corrparity',nan(nTrials,1),'deltaNote',nan(nTrials,1));
    ...
    while trialnum <= nTrials
        ...
        if e_log == 1
            ...
            data.deltaNote(trialnum) = toneStim(trialnum).params.deltaNote;
            
            %% Save everything %%
            save(['DataNote/',subInfo],'data'); % task data
            save(['DataNote/',subInfo,'_toneData'],'toneStim'); % stimulus data
    ...
    zip(['DataNote/' subInfo '_mfiles.zip'],code_filenames);

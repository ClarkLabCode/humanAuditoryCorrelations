%% Start of script to run auditory psychophysical experiments

clear all; close all; clc;

%% Experiment:
% Play a bunch of tones to the subject over N trials
% Record participants' responses on each trial (i.e., upward vs downward tone)
% Vary aspects of the stimulus (+/- correlations; +/- displacement, etc.)

%% Data:
% Button presses (up vs down)
% Note: we will also save stimulus info so we could, if needed, "recreate" every trial!
...
while exist(['Data/',subInfo,'.mat'],'file') % make sure no overwriting is happening
...
mainN = 64; % X of trials for EACH "main" trial type in experiment

randomN = 64; % X number of trials of random "probe" stimuli
...
for n = 1:nTrials
    ...
    elseif trialSeq(n) == 5 % TRIAL TYPE 5
        params.corrType = 'ternRandom'; % random
        params.corrParity = 1;
    ...
    toneStim(n) = ternaryFunction(params);
    
    % Set wav filename
    soundFilename{n} = ['stimuli/',subInfo,'/',subInfo,'_tone',num2str(n),'.wav'];

    % Scale to prevent very distorted noises
...
try
    ...
    % Set coordinates for fixation cross
    ...
    while trialnum <= nTrials
        ...
        if e_log == 1
            %% Check for a reaction time timeout %%
            ...
            %% Save everything %%
            save(['Data/',subInfo],'data'); % task data
            save(['Data/',subInfo,'_toneData'],'toneStim'); % stimulus data
    ...
    zip(['Data/' subInfo '_mfiles.zip'],code_filenames);

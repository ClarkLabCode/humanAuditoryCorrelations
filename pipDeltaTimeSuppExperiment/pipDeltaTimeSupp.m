%% Start of script to run auditory psychophysical experiments (delta T experiment with new pip duration)
...
while exist(['DataDeltaTRedo/',subInfo,'.mat'],'file') % make sure no overwriting is happening
...
% Vector of pip delta T values to sweep through
pipdeltaT_vector = [0 .01 .02 .04 .08 .16 .32];

% Initilaize empty vector
tmpSeq = [];

% Create random sequences of trial types (i.e., corr and displacement) and
% delta Ts if the pips (see vector of 7 values above)
...
% Set the fixed stimulus parameters (these are key psychophysical
% variables)
...
params.noteDur = 1/6; % note duration (in sseconds)
params.deltaNote = 1; % delta note
params.deltaT = 1; % detla time
params.pipDur = 1/50; % pip duration
params.dens = 8; % Hz, density of pips per tone, per second
Fs = 2e4; % freq
...
for n = 1:nTrials
    ...
    toneStim(n) = pipDeltaTimeSuppFunction(params);
...
try
    ...
    while trialnum <= nTrials
        ...
        if e_log == 1
            ...
            save(['DataDeltaTRedo/',subInfo],'data'); % task data
            save(['DataDeltaTRedo/',subInfo,'_toneData'],'toneStim'); % stimulus data
    ...
    zip(['DataDeltaTRedo/' subInfo '_mfiles.zip'],code_filenames);

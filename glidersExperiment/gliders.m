%% Start of script to run auditory psychophysical experiments (gliders)
...
while exist(['DataGliders/',subInfo,'.mat'],'file') % make sure no overwriting is happening
...
mainN = 40; % number of trials for each "main" trial type in experiment

% Vector of glider stimulus types
corrTypeNumber_vector = [5 6]; % 5 = converging, 6 = diverging

% Initilaize empty vector
tmpSeq = [];

% Create random sequences of trial types (i.e., correlation and direction) 
% and glider types (see vector of 2 values above)
tmpSeq = [ones(mainN,1); 2*ones(mainN,1); 3*ones(mainN,1); 4*ones(mainN,1)];
tmp = rectpulse(corrTypeNumber_vector,mainN/length(corrTypeNumber_vector))';
tmpCorrTypeNumberSeq = repmat(tmp',1,4)';
...
corrTypeNumberSeq = tmpCorrTypeNumberSeq(randSeq);
...
params.REPEATOCTAVE = 0; % don't repeat over octaves
Fs = 2e4; % freq
...
for n = 1:nTrials
   ...
   % Vary glider type
   params.corrTypeNumber = corrTypeNumberSeq(n);
    ...
    toneStim(n) = glidersFunction(params);
...
try
    ...
    data = struct('rt',nan(nTrials,1),'choice',nan(nTrials,1),'rt_violation',nan(nTrials,1),...
        ...
        'direction',nan(nTrials,1),'corrParity',nan(nTrials,1),'corrTypeNumber',nan(nTrials,1));
    ...
    while trialnum <= nTrials
        ...
        if e_log == 1
            ...
            data.corrTypeNumber(trialnum) = toneStim(trialnum).params.corrTypeNumber;
            
            %% Save everything %%
            save(['DataGliders/',subInfo],'data'); % task data
            save(['DataGliders/',subInfo,'_toneData'],'toneStim'); % stim data
    ...
    zip(['DataGliders/' subInfo '_mfiles.zip'],code_filenames);
